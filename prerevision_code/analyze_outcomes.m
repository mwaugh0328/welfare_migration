function [targets] = analyze_outcomes(params, flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
R = 0.95; % Storage technology that looses value over time. We are thinking currency. Citation for the 0.96 number?
...
m = 0.16; % This is the permenant moving cost. 
...
%         rural_urban_value(grid, params, trans_shocks, trans_mat);
...
assets_temp_cash = zeros(n_asset_states,n_shocks,2,n_types);
move_temp_cash = zeros(n_asset_states,n_shocks,2,n_types);
cons_eqiv_cash = zeros(n_asset_states,n_shocks,2,n_types);
...
% First, perform the field experiment...
    ...
    [assets_temp(:,:,:,xxx), move_temp(:,:,:,xxx),... 
        cons_eqiv(:,:,:,xxx)] = field_experiment(grid, params, trans_shocks, trans_mat, vguess(:,:,:,xxx));

    [assets_temp_cash(:,:,:,xxx), move_temp_cash(:,:,:,xxx),... 
       cons_eqiv_cash(:,:,:,xxx)] = cash_experiment(grid, params, trans_shocks, trans_mat, vguess(:,:,:,xxx));
%     
%     [assets_temp_cash(:,:,:,xxx), move_temp_cash(:,:,:,xxx),... 
...
%     states_panel(:,1,xxx) = randi(3,length(states_panel(:,1,xxx)),1);
    ...
    [sim_cash_panel(:,:,:,xxx), ~]...
        = experiment_driver_mex(assets(:,:,:,xxx), move(:,:,:,xxx), ...
          assets_temp_cash(:,:,:,xxx), move_temp_cash(:,:,:,xxx), cons_eqiv_cash(:,:,:,xxx),...
          params_sim, trans_shocks, monga_index, states_panel(:,:,xxx), pref_shocks, sim_panel(:,:,xxx));
...
rural_cntr = data_panel_cntr(:,4,1)==1 & data_panel_expr(:,expr_ind,1)==1;
