...
edges_income = prctile(income_assets(:,income_or_assets),income_prct);
...
for xxx = 1:length(edges_income)-1
        ...
        income_yes = edges_income(xxx) <= income_assets(:,income_or_assets) & ... 
        income_assets(:,income_or_assets) < edges_income(xxx+1) ;        
...
%         test_migrate = (test ==1) & (income_assets(:,4) == 1);
        ...
        moving_cost_bin(xxx) = sum(income_assets(test,6))./sum(income_assets(test,4));
...
%         urban_bin(xxx) = mean(urban_prd(test_migrate));
        expr_bin(xxx) = mean(income_assets(test,5));
