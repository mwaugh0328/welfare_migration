function [z , zprob] = pareto_approx_alt_v2(N, theta)
...
for xxx = 1:N
    z(xxx) = zspace(xxx) + zstep(xxx)/2;
     if xxx == N
         ...
         % This is the expected value from the 90th percentile on...
     end
end 
...
% zprob(end) = 1 - sum(zprob(1:end-1));
% 
% zfin = xmin./(1-sum(zprob(1:end-1))).^(1./theta);
% 
% z(end) = zfin.*(theta./(theta-1));
