function [panel, states] = simmulate_prefshock_welfare(assets_policy, move_policy, ...
                    params, perm_types, trans_shocks, shock_states, pref_shock, moveshock, vfun)%#codegen
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Rural NO-EXPERIENCE
    ...
    if location(xxx) == 1 % rural area
        ...
        [labor_income(xxx,1), tax(xxx,1), production(xxx,1)] = labor_income_tax(z_rural.*r_shocks(shock_states(xxx)), params.tax, 0);
        ...
        if move_seasn(xxx,1) == 1
            ...
            fiscal_cost(xxx,1) = params.m_fiscal(asset_state);
        elseif move(xxx,1) == 1
            location(xxx+1) = 5;
            move_cost(xxx,1) = m;            
        end
...
% these are the seasonal moves. 
    ...
    elseif location(xxx) == 2 % seasonal movers....
        ...
        [labor_income(xxx,1), tax(xxx,1), production(xxx,1)] = labor_income_tax(z_urban.*u_shocks(shock_states(xxx)), params.tax, 1);
...
% Experinced Seaonsal movers...
    ...
    elseif location(xxx) == 4
        ...
        %[labor_income(xxx,1), tax(xxx,1)] = labor_income_tax(z_urban.*u_shocks(shock_states(xxx)), params.tax);
        [labor_income(xxx,1), tax(xxx,1), production(xxx,1)] = labor_income_tax(z_urban.*u_shocks(shock_states(xxx)), params.tax, 1);
